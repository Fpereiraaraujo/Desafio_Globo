...
class SubscriptionRenewalProcessingServiceTest {
	...
	@Mock
	private RenewalPaymentGateway paymentGateway;
	...
	void approvesPaymentAndMovesSubscriptionToNextCycle() {
		...
		verify(paymentGateway).charge(subscription, subscription.getId() + ":2026-09-12:attempt:1");
	}

	@Test
	void keepsSubscriptionOnPaymentFailure() {
		Subscription subscription = subscription();
		when(paymentGateway.charge(subscription, subscription.getId() + ":2026-09-12:attempt:3"))
