...
public class ProcessRenewalUseCaseImp implements ProcessRenewalUseCase {
	...
	public RenewalProcessingResult execute(ProcessRenewalParam param) {
		...
		if (param.attemptNumber() < 1 || param.attemptNumber() > RenewalPolicy.defaultPolicy().maxAttempts()) {
		...
		var entity = subscriptionRepository.findByIdForUpdate(param.subscriptionId())
			.orElseThrow(() -> new ResourceNotFoundException("Subscription not found"));
		var subscription = SubscriptionDatabaseMapper.toDomain(entity);
		var existingAttempt = renewalAttemptRepository.findBySubscriptionIdAndRenewalDateAndAttemptNumber(
			subscription.getId(), renewalDate, param.attemptNumber()
		);
		if (existingAttempt.isPresent()) {
			var attempt = RenewalAttemptDatabaseMapper.toDomain(existingAttempt.get());
			var transaction = paymentTransactionRepository.findByIdempotencyKey(attempt.getIdempotencyKey())
				.map(PaymentTransactionDatabaseMapper::toDomain)
				.orElseThrow(() -> new IllegalStateException("Renewal attempt has no payment transaction"));
			return new RenewalProcessingResult(attempt, transaction, subscription);
		}
		if (subscription.getStatus() != SubscriptionStatus.ACTIVE || !subscription.getExpirationDate().equals(renewalDate)) {
			throw new IllegalStateException("Subscription is not eligible for renewal on this date");
		}
